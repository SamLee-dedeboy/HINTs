...
def ravasz(G, attr_dict, D=None):
    ...
    def similarity(G, A, K, D):
        ...
        print("generating combinations: ", n, n)
        CS = np.zeros((n, n))
        print("generating neighbors: ", n)
...
def read_data(dataset, data_type):
    if data_type == 'entity':
        entity_data = json.load(open(f'data/result/{dataset}/network/server/entity_explanations_embeddings.json'))
        entity_embeddings = {entity_id: entity['embedding'] for entity_id, entity in entity_data.items()}
        return entity_embeddings
    elif data_type == 'article':
        article_data = json.load(open(f'data/result/{dataset}/network/server/article_embeddings.json'))
...
def main():
    ...
    # read network
    B = nx.node_link_graph(json.load(open(f'test/data/result/{args['dataset']}/network/hgraph.json')))
    H = hnx.Hypergraph.from_bipartite(B)
    if args['data_type'] == 'article':
        # dual_H is for clustering articles, H is for clustering entities
        H = H.dual()
    embeddings = read_data(args['dataset'], args['data_type'])
    ...
    # save_json(partitions, f'test/data/result/{args['dataset']}/network/server/ravasz_partitions_{args['data_type']}.json')

    hierarchies = get_level_transition(renumbered_levels)
    hierarchies = add_dummy_hierarchy(partitions, hierarchies)
    # save_json(hierarchies, f"test/data/result/{args['dataset']}/network/server/ravasz_hierarchies_{args['data_type']}.json")
    end_time = time.time()
    # 180 (30 + 150)s for VisPub article (2632 articles and 412 (164 + 248)s on (5101) entities
