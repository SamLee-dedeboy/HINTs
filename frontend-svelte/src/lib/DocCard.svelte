<script lang=ts>
    ...
    $: clickable = doc.title === ""? "pointer-events-none" : "pointer-events-auto"
    $: highlightedSummary = (() => {
...
<div 
    ...
    hover:shadow-2xl 
    `} 
    style={`border-color: ${getThemeColor(doc.color)}`}
    role=button
    on:click={() => { dispatch('click', {doc_id: doc.id || "", add: !clicked}); clicked=!clicked; }}>
    ...
    <div class='doc-card-content flex flex-col' >
        ...
        <span class="text-left px-1"> Topic: {doc.cluster_label} </span>
        <span class="text-left px-1"> Relevance: {doc.relevance?.toFixed(2)} </span>
        </p>
    </div>
</div>