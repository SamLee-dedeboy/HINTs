...
def query_leaf_topic(nodes, node_type, dataset):
    if node_type == "article":
        example = json.load(open(f"{dataset}/data/raw/example_article.json"))
        summaries = [node["summary"] for node in nodes]
        enc = tiktoken.encoding_for_model("gpt-3.5-turbo-0125")
        max_token_length = 16385
        summaries_message = ""
        for index, summary in enumerate(summaries):
            if len(enc.encode(summaries_message + summary)) > max_token_length - 1000:
                continue
        ...
        if dataset == "VisPub":
            messages = [
                ...
                },
                {
                    "role": "system",
                    "name": "example_user",
                    "content": example["leaf"]["summaries"],
                },
                {
                    "role": "system",
                    "name": "example_system",
                    "content": example["leaf"]["topic"],
                },
    ...
    if dataset == "AllTheNews":
        messages = [
            ...
            },
            {
                "role": "system",
                "name": "example_user",
                "content": example["non-leaf"]["summaries"],
            },
            {
                "role": "system",
                "name": "example_system",
                "content": example["non-leaf"]["topic"],
            },
...
def dfs(
    ...
    if hierarchy["key"] in topic_dict:
        return  # if already have a topic, skip. This happens when continuing from a break point
    ...
    if level == 1:  # at level 1, use the children (leaf nodes) to generate a topic
        ...
        if node_type == "article":
            # article
            cluster_subtopics = [
                topic_dict[child["key"]] for child in hierarchy["children"]
            ]
            gpt_topic = query_cluster_topic(cluster_subtopics, sample_articles, dataset)
        else:  # entity
            cluster_subtopics = [
                topic_dict[child["key"]].split(",") for child in hierarchy["children"]
            ]
            cluster_subtopics = [
                item.strip() for sublist in cluster_subtopics for item in sublist
            ]  # flatten
            gpt_topic = query_leaf_topic(
                cluster_subtopics, node_type="entity", dataset=dataset
            )
        # record the result
        topic_dict[hierarchy["key"]] = gpt_topic
        print(hierarchy["key"], gpt_topic)
...
def main():
    ...
    node_dict = json.load(
        relative_path(
            open(f"{args['dataset']}/data/result/network/{data_type_plural}.json")
        )
    )
    result_file_path = relative_path(
        f"{args['dataset']}/data/result/server/hierarchical_topics_{data_type_plural}.json"
    )
    try:
        ...
        topic_dict = {}
    sampleFlag = (
        args["data_type"] == "article"
    )  # entity hierarchies do not need sampling
    print("generating hierarchical topics...")
    topic_dict = add_hierarchical_topic(
        hierarchy,
        partitions,
        node_dict,
        args["data_type"],
