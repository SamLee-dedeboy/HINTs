...
def convert_to_graph(H):
    ...
    for s_component in component_subgraphs:
        ...
        for e in cc.es:
            if (
                index2id_dict[e.source] in deleted_vertices
                or index2id_dict[e.target] in deleted_vertices
            ):
        ...
        if len(cc.es) != 0:
            for e in cc.es:
                weights[cc.vs[e.source]["name"]][cc.vs[e.target]["name"]]["weight"] = e[
                    "weight"
                ]
                weights[cc.vs[e.target]["name"]][cc.vs[e.source]["name"]]["weight"] = e[
                    "weight"
                ]
...
def ravasz(G, attr_dict, D=None):
    ...
    def weighted_common_neighbors(N, A, i, j):
        ...
        return sum([A[i][v] for v in common_neighbors]) + sum(
            [A[j][v] for v in common_neighbors]
        )
    ...
    def fusion_matrix_adjacency(A, comms):
        ...
        for comm1, vertices1 in comms.items():
            for comm2, vertices2 in comms.items():
                new_weights[comm1][comm2]["weight"] = calculate_weights(
                    vertices1, vertices2, A
                )
    ...
    while True:
        ...
        for v in G.vs:
            ...
            most_similar_node = max(
                range(len(similarity_matrix[v.index])),
                key=similarity_matrix[v.index].__getitem__,
            )
        ...
        ori_graph_comms_dict = reverse_index(ori_graph_partition)
        print(
            "most similar nodes: ", len(most_similar_nodes), len(ori_graph_comms_dict)
        )
        ...
        comms_dict = reverse_index(P)
        print(
            "total nodes in communities:",
            sum([len(x) for x in ori_graph_comms_dict.values()]),
        )
        ...
        if G.vcount() < 10 or G.vcount() == c_G.vcount():
            break
        print("pass done. ")
        G = c_G
        A = c_A
    ...
    for i in range(len(levels[0]) - 1):
        for v, transitions in levels.items():
            ...
            if trans_children_title not in nested_comms:
                ...
                elif trans_children_title not in [
                    child["title"]
                    for child in nested_comms[trans_parent_title]["children"]
                ]:
                    nested_comms[trans_parent_title]["children"].append(
                        nested_comms[trans_children_title]
                    )
            else:
...
def add_dummy_hierarchy(partitions, hierarchies):
    ...
    for node_id, dummy_cluster_label in first_partition.items():
        ...
        second_level_children_dict[str(parent_cluster_label)].append(
            dummy_cluster_label
        )
...
def read_embeddings(dataset, data_type, relative_path=lambda x: x):
    if data_type == "entity":
        entity_data = json.load(
            open(
                relative_path(
                    f"{dataset}/data/result/server/keyword_explanations_embeddings.json"
                )
            )
        )
        entity_embeddings = {
            entity_id: entity["embedding"] for entity_id, entity in entity_data.items()
        }
        return entity_embeddings
    elif data_type == "article":
        article_data = json.load(
            open(relative_path(f"{dataset}/data/result/server/article_embeddings.json"))
        )
        article_embeddings = {
            str(article["doc_id"]): article["embedding"]
            for _, article in article_data.items()
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Clustering with Ravasz Algorithm on article/entity in VisPub/AllTheNews."
    )
    parser.add_argument(
        "-dataset", help="The dataset to parse.", choices=["VisPub", "AllTheNews"]
    )
    parser.add_argument(
        "-data_type", help="The type of hypergraph", choices=["article", "entity"]
    )
    args = vars(parser.parse_args())
    # read network
    B = nx.node_link_graph(
        json.load(
            open(relative_path(f'{args["dataset"]}/data/result/network/hgraph.json'))
        )
    )
    ...
    if args["data_type"] == "article":
        ...
        H = H.dual()
    embeddings = read_embeddings(args["dataset"], args["data_type"], relative_path)
    ...
    save_json(
        partitions,
        relative_path(
            f'{args["dataset"]}/data/result/server/ravasz_partitions_{args["data_type"]}.json'
        ),
    )
    ...
    save_json(
        hierarchies,
        relative_path(
            f'{args["dataset"]}/data/result/server/ravasz_hierarchies_{args["data_type"]}.json'
        ),
    )
    end_time = time.time()
