...
def read_embeddings(dataset, data_type):
    if data_type == 'entity':
        entity_data = json.load(open(f'data/result/server/keyword_explanations_embeddings.json'))
        entity_embeddings = {entity_id: entity['embedding'] for entity_id, entity in entity_data.items()}
        return entity_embeddings
    elif data_type == 'article':
        article_data = json.load(open(f'data/result/server/article_embeddings.json'))
        article_embeddings = {str(article['doc_id']): article['embedding'] for article in article_data}
...
def main():
    ...
    # read network
    B = nx.node_link_graph(json.load(open(f'data/result/network/hgraph.json')))
    ...
    save_json(partitions, f'data/result/server/ravasz_partitions_{args["data_type"]}.json')

    hierarchies = get_level_transition(renumbered_levels)
    hierarchies = add_dummy_hierarchy(partitions, hierarchies)
    save_json(hierarchies, f'data/result/server/ravasz_hierarchies_{args["data_type"]}.json')
    end_time = time.time()
