...
def main():
    ...
    print("reading data...")
    hierarchy = json.load(open(f"data/result/server/ravasz_hierarchies_{args['data_type']}.json"))
    partitions = json.load(open(f"data/result/server/ravasz_partitions_{args['data_type']}.json"))
    data_type_plural = "entities" if args['data_type'] == "entity" else "articles"
    node_dict = json.load(open(f"data/result/network/{data_type_plural}.json"))
    result_file_path = f"data/result/server/hierarchical_topics_{data_type_plural}.json"
    try:
